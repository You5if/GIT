...
export class BrdStateTComponent implements OnInit {
    ...
    lastState: BrdStateModel = {
      ...
      AddC: true,
      cardArray: [],
      boardTemplateId: +localStorage.getItem(this._globals.baseAppName + '_boardTId'),
      entryMode: 'A',
      active: true,
      readOnly: true,
      auditColumns: {
  ...
  ngOnInit() {
    this.Add = true
      ...
      this.brdstateservice.getBoardEntry(+localStorage.getItem(this._globals.baseAppName + '_boardTId')).subscribe((res) =>{
  ...
  refreshMe() {
    ...
    //     }
    //   );
    this.brdstateservice.getBoardStateEntry(+localStorage.getItem(this._globals.baseAppName + '_boardTId')).subscribe((result1) => {
      this.states = result1
      this.states.forEach((brdSt) => {
        brdSt.AddC = true
        brdSt.cardArray = []
      })
      this.brdstateservice.getCards(+localStorage.getItem(this._globals.baseAppName + '_boardTId')).subscribe((result2) => {
      this.states.forEach((brdSt) => {
          console.log(result2);
            result2.forEach((r) => {
              r.auditColumns = {
